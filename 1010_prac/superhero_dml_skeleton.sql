...
-- 4.4 나이가 10번째로 많은 사람
SELECT * FROM superhero
ORDER BY 나이 DESC LIMIT 9, 1;

-- 4.5 나이가 10번째 ~ 15번째 사람들
SELECT * FROM superhero
ORDER BY 나이 DESC LIMIT 9, 6;
...
-- 4.6 전체 데이터 수를 구하여라
SELECT COUNT(*) AS 전체데이터수
FROM superhero;

-- 조건문과 함께 활용하기
-- 4.7 직업이 악당인 사람의 수
SELECT COUNT(*) AS 악당수 FROM superhero
WHERE 직업 = '악당';

-- 평균 구하기(AVG)

-- 4.8 영웅들의 평균 나이를 구하여라.
SELECT AVG(나이) AS 평균나이
FROM superhero;

-- IF 소속회사가 마블인 경우
SELECT AVG(나이) AS 마블평균나이
FROM superhero
WHERE 소속회사 = '마블';

-- 문자열은 평균값이 아닌 다른 값을 출력한다.
-- 버그는 안나니까 주의
SELECT AVG(국적) AS 평균국적
FROM superhero;
...
-- 4.10 소속회사 별로 그룹화 하여, 
-- 40살 이상인 데이터만 평균을 구하여라.
SELECT 소속회사, AVG(나이) AS 평균나이
FROM superhero
WHERE 나이 >= 40 GROUP BY 소속회사;

-- 그룹화 후에 조건주기(HAVING)
-- 4.11 평균 나이가 40살 이상인 소속회사를 구하여라
SELECT 소속회사, AVG(나이) AS 평균나이
FROM superhero
GROUP BY 소속회사 HAVING 평균나이 >= 40;
